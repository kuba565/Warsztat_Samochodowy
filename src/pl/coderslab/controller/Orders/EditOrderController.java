...
public class EditOrderController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		req.getRequestDispatcher("/WEB-INF/Orders/editOrder.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Order order = new Order(id, arrivalString, repairBeginStringPlan, repairBeginStringReal, assignedWorker, problemDesc, repairDesc, status, vehicleRepaired, customerRepairCost, partsCost, wrkHourCost, wrkHours);
		if (cDao.update(order)) {
			request.setAttribute("cDao", cDao);

			response.sendRedirect("Orders");
		} else {
			response.sendRedirect("EditOrder");
